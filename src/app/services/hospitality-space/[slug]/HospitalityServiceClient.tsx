...
import type { HospitalityService } from "@/types/service";
...
interface HospitalityServiceClientProps {
  service: HospitalityService;
  related: typeof PROJECTS;
}

export default function HospitalityServiceClient({ service, related }: HospitalityServiceClientProps) {
  ...
  return (
    <>
      <style>{`
        ...
        .hsp-scope-pill:hover { background: rgba(201,169,110,0.08); border-color: var(--gold); transform: scale(1.03); }
        ...
        .hsp-project-card:hover { transform: translateY(-8px) scale(1.02); box-shadow: 0 24px 60px rgba(0,0,0,0.5), 0 0 0 1px rgba(201,169,110,0.15); }
        .hsp-project-card img { width: 100%; height: 100%; object-fit: cover; transition: transform 0.6s ease; }
        .hsp-project-card:hover img { transform: scale(1.08); }
        .hsp-project-overlay {
        ...
        .hsp-project-card:hover .hsp-project-overlay { opacity: 1; }

        .hsp-feature-item {
          display: flex; align-items: flex-start; gap: 12px;
          padding: 14px 0; border-bottom: 1px solid rgba(255,255,255,0.06);
        }
        .hsp-feature-item:last-child { border-bottom: none; }
        ...
        .hsp-cta-primary { background: var(--gold); color: #fff; border: 1.5px solid var(--gold); }
        .hsp-cta-primary:hover { background: var(--gold-dark); border-color: var(--gold-dark); transform: scale(1.04); }
        .hsp-cta-ghost { background: transparent; color: rgba(255,255,255,0.8); border: 1.5px solid rgba(255,255,255,0.25); }
        .hsp-cta-ghost:hover { border-color: var(--gold); color: var(--gold); transform: scale(1.04); }

        .hsp-divider { width: 48px; height: 1px; background: var(--gold); margin: 24px 0; }

        .hsp-step { display: flex; align-items: flex-start; gap: 20px; padding: 22px 0; border-bottom: 1px solid rgba(255,255,255,0.06); }
        .hsp-step:last-child { border-bottom: none; }
        .hsp-step-num { flex-shrink: 0; width: 40px; height: 40px; border: 1px solid rgba(201,169,110,0.35); display: flex; align-items: center; justify-content: center; transition: border-color 0.3s, background 0.3s; }
        .hsp-step-num:hover { border-color: var(--gold); background: rgba(201,169,110,0.08); }

        .hsp-stat-card { padding: 36px 24px; text-align: center; border: 1px solid rgba(255,255,255,0.07); background: rgba(255,255,255,0.02); transition: border-color 0.3s; }
        .hsp-stat-card:hover { border-color: rgba(201,169,110,0.3); }

        @media (max-width: 768px) {
          section { padding-left: 20px !important; padding-right: 20px !important; }
          .hsp-two-col { grid-template-columns: 1fr !important; gap: 48px !important; }
          .hsp-three-col { grid-template-columns: 1fr !important; }
          .hsp-stats { grid-template-columns: 1fr !important; }
        }
      `}</style>
      ...
      <section style={{ position: "relative", minHeight: "72vh", display: "flex", alignItems: "flex-end", background: service.heroGradient, paddingTop: 100, overflow: "hidden" }}>
        ...
        <div style={{ position: "relative", width: "100%", maxWidth: 1200, margin: "0 auto", padding: "0 48px 80px" }}>
          <nav style={{ display: "flex", alignItems: "center", gap: 8, fontFamily: "var(--font-body)", fontSize: 11, letterSpacing: 1.5, textTransform: "uppercase", marginBottom: 32, ...fade(0) }}>
            ...
            <Link href="/services/hospitality-space" style={{ color: "rgba(255,255,255,0.4)", textDecoration: "none" }}>Hospitality</Link>
            <span style={{ color: "rgba(255,255,255,0.2)" }}>›</span>
          ...
          <div style={{ display: "flex", gap: 14, flexWrap: "wrap", ...fade(480) }}>
            <Link href="/contact" className="hsp-cta-btn hsp-cta-primary">
              Book a Consultation
            ...
            <Link href="/projects" className="hsp-cta-btn hsp-cta-ghost">View Projects</Link>
          </div>
        </div>
      </section>

      <section style={{ background: "#0a0a0a", padding: "0 48px" }}>
        <div className="hsp-stats" style={{ maxWidth: 1200, margin: "0 auto", display: "grid", gridTemplateColumns: "repeat(3,1fr)", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
          {[
            ...
            return (
              <div key={label} ref={refs[index] as React.Ref<HTMLDivElement>} className="hsp-stat-card" style={{ opacity: inViews[index] ? 1 : 0, transform: inViews[index] ? "translateY(0)" : "translateY(32px)", transition: `opacity 0.7s ease ${delay}ms, transform 0.7s ease ${delay}ms` }}>
      ...
      <section style={{ background: "#111111", padding: "96px 48px" }}>
        <div className="hsp-two-col" style={{ maxWidth: 1200, margin: "0 auto", display: "grid", gridTemplateColumns: "1fr 1fr", gap: "64px 80px", alignItems: "start" }}>
          ...
          <AnimatedSection delay={0}>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 10 }}>
              {service.process.map((item) => (
                <span key={item} className="hsp-scope-pill">{item}</span>
              ))}
            </div>
          </AnimatedSection>
          ...
          <AnimatedSection delay={150}>
            ...
            <div style={{ listStyle: "none", padding: 0, margin: 0 }}>
              {service.features.map((feat, i) => (
                <motion.div key={feat} className="hsp-feature-item" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6, delay: i * 0.08 }}>
      ...
      <section style={{ background: "#0d0d0d", padding: "96px 48px" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          ...
          <div className="hsp-two-col" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 80px" }}>
            {service.process.map((step, i) => (
              <motion.div key={i} className="hsp-step" initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.65, delay: i * 0.09 }}>
                <div className="hsp-step-num">
      ...
      {related.length > 0 && (
        <section style={{ background: "#111", padding: "96px 48px" }}>
          <div style={{ maxWidth: 1200, margin: "0 auto" }}>
            ...
            <div className="hsp-three-col" style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 2 }}>
              {related.map((project, i) => (
                <motion.div key={project.id} initial={{ opacity: 0, y: 48 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: i * 0.12 }}>
                  <Link href={`/projects/${project.id}`} className="hsp-project-card">
                    <img src={project.images[0]} alt={project.title} loading="lazy" />
                    <div className="hsp-project-overlay">
                      <div>
            ...
            <AnimatedSection delay={200}>
              <div style={{ marginTop: 40, textAlign: "center" }}>
                <Link href="/projects?category=hospitality" className="hsp-cta-btn hsp-cta-ghost">
        ...
        <div style={{ position: "relative", maxWidth: 640, margin: "0 auto" }}>
          <AnimatedSection delay={0}>
            ...
            <div style={{ display: "flex", gap: 16, justifyContent: "center", flexWrap: "wrap" }}>
              ...
              <Link href="/projects" className="hsp-cta-btn hsp-cta-ghost">View Our Portfolio</Link>
            </div>
          </AnimatedSection>
        </div>
      </section>
